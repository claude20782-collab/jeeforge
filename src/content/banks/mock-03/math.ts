...
**Why the others are wrong:** (A) x + y = 1 misses the midpoint; (B) x − y = 0 is the bisector for points like $1$ and $i$; (C) is a generic parallel line.`,
    ...
    diagram: {
      ...
      yRange: [-3, 3],
      elements: [
        { type: 'point', x: 1, y: 0, label: '1 (1, 0)', labelPos: 'NE' },
        { type: 'point', x: 0, y: -1, label: '−i (0, −1)', labelPos: 'SE' },
        { type: 'segment', from: [1, 0], to: [0, -1], label: '', color: 'var(--chart-3)', dashed: true },
        { type: 'line', from: [-2.5, 2.5], to: [2.5, -2.5], label: '', color: 'var(--gold)' },
        { type: 'point', x: 0.5, y: -0.5, label: 'midpoint', labelPos: 'W' },
        { type: 'label', x: 2.6, y: -2.4, text: 'locus: x + y = 0', color: 'var(--gold)' },
      ],
      square: true,
      showGrid: false,
      title: '|z − 1| = |z + i|',
...
**Why the others are wrong:** (A) $-I = A^2$ itself; (B) $A$ is invertible ($\\det A = 1$), so no power can be the zero matrix; (D) — the cycle $I, A, -I, -A$ repeats, and $64 \\equiv 0 \\pmod 4$ lands exactly on $I$.`,
    ...
    diagram: {
      kind: 'table',
      headers: ['Power', 'Rotation by', 'Result'],
      rows: [
        ['A¹', '90°', 'A'],
        ['A²', '180°', '−I'],
        ['A³', '270°', '−A'],
      ...
      caption: 'powers of the 90° rotation matrix',
      highlightCells: [[4, 2]],
    },
  },
  {
...
**Why the others are wrong:** (A) 2 forgets one factor of 2; (B) 4 takes only the inner squaring; (D) 16 squares the coefficient twice. Each is a plausible slip in this standard chain.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'y = (1 − cos 4x)/x² near 0',
      xAxis: { label: 'x', min: -0.6, max: 0.6, ticks: [-0.4, 0, 0.4] },
      yAxis: { label: 'y', min: 4, max: 9, ticks: [8] },
      curves: [
        { type: 'curve', color: 'var(--gold)', points: [[-0.55, 5.2], [-0.4, 6.2], [-0.25, 7.09], [-0.12, 7.76], [-0.05, 7.95], [0.05, 7.95], [0.12, 7.76], [0.25, 7.09], [0.4, 6.2], [0.55, 5.2]] },
        { type: 'line', color: 'var(--chart-2)', points: [[-0.6, 8], [0.6, 8]], dashed: true },
      ],
      markers: [{ x: 0, y: 8, label: 'k = 8' }],
    },
  },
  {
    subject: 'MATHEMATICS', section: 'A',
...
**Why the others are wrong:** (B) swaps the y-values; (C) picks convenient-looking points not on the horizontal-tangent list; (D) repeats the same x (impossible — the two stationary points differ in sign of $x$).`,
    ...
    diagram: {
      kind: 'graph',
      title: 'y = x³ − 3x + 4',
      xAxis: { label: 'x', min: -2.5, max: 2.5, ticks: [-1, 1] },
      yAxis: { label: 'y', min: -5, max: 13, ticks: [2, 6] },
      curves: [
        { type: 'curve', color: 'var(--gold)', points: [[-2.5, -4.1], [-2, 2], [-1, 6], [0, 4], [1, 2], [2, 6], [2.5, 12.1]] },
      ],
      markers: [
        { x: 1, y: 2, label: '(1, 2)' },
        { x: -1, y: 6, label: '(−1, 6)' },
...
**Why the others are wrong:** (B) has intercepts of opposite sign; (C) doesn't pass through (2, 3) ($2+3 \\neq 1$); (D) $3x + 2y = 12$ has unequal intercepts (4 and 6) — it passes through the point but violates the equal-intercept condition.`,
    ...
    diagram: {
      ...
      yRange: [-1, 6],
      elements: [
        { type: 'line', from: [5, 0], to: [0, 5], label: '', color: 'var(--gold)' },
...
**Why the others are wrong:** (A) x = −2 mirrors it; (B) picks the vertical coordinate; (D) invents a slanted line.`,
    ...
    diagram: {
      ...
      yRange: [-3, 3],
      elements: [
        { type: 'circle', cx: 0, cy: 0, r: 2, label: 'x² + y² = 4' },
        { type: 'circle', cx: 3, cy: 0, r: 1, label: '' },
        { type: 'point', x: 2, y: 0, label: '(2, 0)', labelPos: 'NE' },
        { type: 'line', from: [2, -2.6], to: [2, 2.6], label: '', color: 'var(--gold)' },
        { type: 'label', x: 2.3, y: 2.2, text: 'radical axis x = 2', color: 'var(--gold)' },
      ],
      square: true,
      showGrid: false,
    },
...
**Why the others are wrong:** (A)/(C) put the focus on the y-axis (confusing with $x^2 = 4ay$-type parabolas); (B) takes the directrix position $(-3, 0)$.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'y² = 12x',
      xAxis: { label: 'x', min: -4, max: 4, ticks: [-3, 3] },
      yAxis: { label: 'y', min: -6, max: 6, ticks: [-3, 3] },
      curves: [
        { type: 'curve', color: 'var(--gold)', points: [[3, -6], [1.33, -4], [0.33, -2], [0, 0], [0.33, 2], [1.33, 4], [3, 6]] },
        { type: 'line', color: 'var(--chart-2)', points: [[-3, -6], [-3, 6]], dashed: true },
      ],
      markers: [
        { x: 3, y: 0, label: 'F (3, 0)' },
        { x: -3, y: 5, label: 'directrix x = −3', color: 'var(--chart-2)' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) 4 is the shorter leg; (C) 10 is the diameter, not the radius; (D) 2.5 is half the shorter leg.`,
    ...
    diagram: {
      ...
      yRange: [-6, 7],
      elements: [
        { type: 'circle', cx: 4, cy: 3, r: 5, label: '', color: 'var(--chart-2)' },
        { type: 'polygon', points: [[0, 0], [8, 0], [0, 6]], label: '', color: 'var(--gold)' },
        { type: 'point', x: 0, y: 0, label: 'A', labelPos: 'SW' },
        { type: 'point', x: 8, y: 0, label: 'B', labelPos: 'SE' },
        { type: 'point', x: 0, y: 6, label: 'C', labelPos: 'NW' },
        { type: 'point', x: 4, y: 3, label: 'O = midpoint of BC', labelPos: 'N' },
        { type: 'segment', from: [4, 3], to: [8, 0], label: 'R = 5', color: 'var(--chart-2)', dashed: true },
      ],
      square: true,
      showGrid: false,
      title: 'hypotenuse BC = 10 is the diameter',
    },
  },
...
Only the $z$-separation survives — the $x$ and $y$ offsets can be absorbed by sliding along the lines.`,
    ...
    diagram: {
      ...
      lines: [
        ...
        { from: [2, 2, 3], to: [2, 4, 5], label: '', color: 'var(--chart-3)', dashed: true },
      ],
      points: [
        { x: 1, y: 2, z: 3, label: '(1,2,3)', color: 'var(--gold)' },
        { x: 2, y: 4, z: 5, label: '(2,4,5)', color: 'var(--chart-2)' },
        { x: 2, y: 2, z: 3, label: 'foot', color: 'var(--chart-3)' },
      ],
      vectors: [
