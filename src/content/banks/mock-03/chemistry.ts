...
**Why the others are wrong:** (A) 3:4 inverts; (B) 1:1 assumes $n^2$ and $Z$ scale equally (here $4/1 \\ne 9/3$); (D) 9:4 compares the $n^2$ values alone.`,
    ...
    diagram: {
      kind: 'table',
      headers: ['Species', 'n', 'Z', 'rₙ = 0.529·n²/Z (Å)'],
      rows: [
        ['H', 2, 1, 2.116],
        ['Li²⁺', 3, 3, 1.587],
      ],
      caption: 'ratio r(H) : r(Li²⁺) = 4 : 3',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) tetrahedral would need 4 domains and no lone pair (that is $\\text{SiF}_4$); (C) square planar belongs to $sp^3d^2$ with **2** lone pairs ($\\text{XeF}_4$); (D) trigonal bipyramidal is the *electron-pair* geometry — but with one lone pair the *molecular* shape distorts to see-saw.`,
    ...
    diagram: {
      kind: 'molecule',
      atoms: [
        { sym: 'S', x: 160, y: 110, label: 'S' },
        { sym: 'F', x: 160, y: 40, label: 'F (axial)' },
        { sym: 'F', x: 160, y: 180, label: 'F (axial)' },
        { sym: 'F', x: 90, y: 110, label: 'F (eq)' },
        { sym: 'F', x: 230, y: 132, label: 'F (eq)' },
      ],
      bonds: [
        { a: 0, b: 1, order: 1 }, { a: 0, b: 2, order: 1 },
        { a: 0, b: 3, order: 1 }, { a: 0, b: 4, order: 1 },
      ],
      lonePairs: [{ atom: 0, count: 1, angles: [35] }],
      caption: 'SF₄ — see-saw (lone pair equatorial)',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) 100 K mixes kJ with J ($40/100 = 0.4$-style blunder ×100); (B) 250 K forgets the sign conventions; (D) 500 K divides by $\\Delta S/2$-style values.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'ΔG vs T for this reaction',
      xAxis: { label: 'T (K)', min: 0, max: 800, ticks: [400] },
      yAxis: { label: 'ΔG (kJ/mol)', min: -20, max: 40, ticks: [0] },
      curves: [
        { type: 'line', color: 'var(--gold)', points: [[0, -40], [400, 0], [800, 40]] },
      ],
      markers: [{ x: 400, y: 0, label: 'T = ΔH/ΔS = 400 K' }],
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (B) 90 mm Hg takes the simple average of 100 and 80 (assumes $x_A = x_B = 0.5$); (C) 95 mm Hg weights A too heavily; (D) 80 mm Hg is the pressure of pure B (as if A were non-volatile).`,
    ...
    diagram: {
      kind: 'graph',
      title: 'p vs x_A (ideal, A more volatile)',
      xAxis: { label: 'x_A', min: 0, max: 1, ticks: [0.25, 0.5, 1] },
      yAxis: { label: 'p (mm Hg)', min: 60, max: 110, ticks: [80, 85, 100] },
      curves: [
        { type: 'line', color: 'var(--chart-2)', points: [[0, 80], [1, 0]] },
        { type: 'line', color: 'var(--chart-3)', points: [[0, 0], [1, 100]] },
        { type: 'line', color: 'var(--gold)', points: [[0, 80], [1, 100]] },
      ],
      markers: [{ x: 0.25, y: 85, label: 'our mixture: 85 mm' }],
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) HClO is the weakest of the set (hypochlorous acid, pKa ≈ 7.5); (B) and (C) sit in between (HClO₃ is strong but still below HClO₄, pKa ≈ −8).`,
    ...
    diagram: {
      kind: 'table',
      headers: ['Acid', 'Cl oxidation state', 'Acid strength'],
      rows: [
        ['HClO', '+1', 'weakest'],
        ['HClO₂', '+3', 'weak'],
        ['HClO₃', '+5', 'strong'],
      ...
      caption: 'more terminal O ⇒ stronger acid',
      highlightCells: [[3, 0], [3, 2]],
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) and (D) are primary-substitution products (~1% each); (C) is the secondary product (~9%) — the numbering trap for 2-bromo-3-methylbutane.`,
    ...
    diagram: {
      kind: 'organic',
      parts: [
        { type: 'chain', x: 40, y: 60, atoms: [{ sym: 'CH₃' }, { sym: 'CH(CH₃)' }, { sym: 'CH₂' }, { sym: 'CH₃' }] },
        { type: 'text', x: 40, y: 100, text: 'H types: 1 tertiary (C-2), 2 secondary (C-3),', bold: false },
        { type: 'text', x: 40, y: 120, text: '9 primary — the tertiary C–H wins with Br₂' },
      ],
      caption: '2-methylbutane skeleton',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (B) Cu/HCl is the **Gattermann** reaction — it also gives chlorobenzene, but in lower yield; JEE convention assigns Sandmeyer ($\\text{Cu}_2\\text{Cl}_2$/HCl) as the standard answer for the diazonium → chloro conversion; (C) HCl alone cannot replace $\\text{N}_2^+$ — no copper catalyst; (D) aqueous $\\text{CuCl}_2$ mainly brings hydrolysis to phenol side-products.`,
    ...
    diagram: {
      kind: 'organic',
      parts: [
        { type: 'ring', x: 100, y: 90, ringSize: 6, aromatic: true, label: '', substituents: [{ position: 0, label: 'N₂⁺Cl⁻' }] },
        { type: 'arrow', x1: 180, y1: 90, x2: 260, y2: 90, label: 'Cu₂Cl₂ / HCl' },
        { type: 'ring', x: 320, y: 90, ringSize: 6, aromatic: true, label: '', substituents: [{ position: 0, label: 'Cl' }] },
        { type: 'text', x: 40, y: 150, text: 'Sandmeyer: −N₂ ↑', bold: false },
      ],
      caption: 'diazonium → chlorobenzene',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (B) 20% halves the CO₂ carbon fraction; (C) 12% H counts the whole water mass as hydrogen; (D) 26.7%/2.2% belong to a different sample entirely.`,
    ...
    diagram: {
      kind: 'apparatus',
      parts: [
        { type: 'tube', x: 40, y: 100, w: 200, h: 44, label: 'combustion tube (CuO, red-hot)' },
        { type: 'arrow', x1: 10, y1: 122, x2: 38, y2: 122, label: 'O₂ + sample' },
        { type: 'arrow', x1: 245, y1: 122, x2: 275, y2: 122, label: '' },
        { type: 'tube', x: 285, y: 100, w: 70, h: 44, label: 'H₂O absorber' },
        { type: 'tube', x: 380, y: 100, w: 70, h: 44, label: 'CO₂ absorber' },
        { type: 'label', x: 300, y: 170, text: 'weigh before & after' },
      ],
      caption: 'Liebig carbon–hydrogen estimation train',
    },
  },
