...
**Why the others are wrong:** (B) inverts the ratio (imagine the 60° shot goes lower!); (C) uses $\\tan$ instead of $\\sin$ ($\\tan^2 30^\\circ = 1/3 \\ne 1/9$ — actually $\\tan 60/\\tan 30 = 3$ squared = 9, a double error); (D) forgets the square.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'trajectories (to scale), u = 20 m/s',
      xAxis: { label: 'x (m)', min: 0, max: 40, ticks: [10, 20, 30] },
      yAxis: { label: 'y (m)', min: 0, max: 16, ticks: [5, 10, 15] },
      curves: [
        { type: 'curve', color: 'var(--gold)', points: [[0, 0], [10, 4.1], [17.3, 5], [25, 3.4], [34.6, 0]], label: '30°' },
        { type: 'curve', color: 'var(--chart-2)', points: [[0, 0], [8.66, 11.25], [17.32, 15], [25.98, 11.25], [34.64, 0]], label: '60°' },
      ],
      markers: [
        { x: 17.3, y: 5, label: 'H₃₀ = 5 m', color: 'var(--gold)' },
...
**Why the others are wrong:** (A) 0.30 m forgets the extra fall $x$ while compressing (uses $mgh = \\tfrac12kx^2$ alone — that gives $x = 0.141$ m, close to but distinct from the true 0.152 m; option A is pure mis-scaling); (C) 0.10 m drops the $mgx$ term AND mis-solves; (D) 0.20 m doubles the correct root.`,
    ...
    diagram: {
      kind: 'fbd',
      bodies: [
        { type: 'ground', x: 30, y: 250, w: 240 },
        { type: 'block', x: 128, y: 60, w: 36, h: 26, label: 'm' },
        { type: 'rod', x: 138, y: 178, w: 16 },
        { type: 'rod', x: 138, y: 198, w: 16 },
        { type: 'rod', x: 138, y: 218, w: 16 },
      ],
      forces: [
        { from: [146, 86], to: [146, 130], label: 'mg', color: 'var(--chart-3)' },
        { from: [146, 208], to: [146, 164], label: 'kx', color: 'var(--chart-2)' },
      ],
      dims: [{ from: [190, 73], to: [190, 178], label: 'h = 1.0 m' }],
    },
  },
  {
    subject: 'PHYSICS', section: 'A',
...
**Why the others are wrong:** (A) 2T would require $g_{eff} = g/4$ (downward acceleration $3g/4$-ish); (B) $T\\sqrt2$ corresponds to $g_{eff} = g/2$ (lift accelerating *down* at $g/2$); (C) $T/2$ needs $g_{eff} = 4g$.`,
    ...
    diagram: {
      kind: 'fbd',
      bodies: [
        { type: 'ground', x: 250, y: 60, w: 60 },
        { type: 'pulley', x: 120, y: 70, r: 12 },
        { type: 'string', x: 120, y: 82, w: 0, h: 90 },
        { type: 'sphere', x: 120, y: 172, r: 18, label: 'm' },
      ],
      forces: [
        { from: [120, 154], to: [120, 110], label: 'T', color: 'var(--chart-2)' },
        { from: [120, 178], to: [120, 216], label: 'mg', color: 'var(--chart-3)' },
        { from: [86, 178], to: [86, 216], label: 'ma', color: 'var(--chart-3)', dashed: true },
        { from: [200, 56], to: [200, 96], label: 'lift a = g ↑', color: 'var(--gold)' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) 100 m/s is the wavelength (frequency forgotten in $v = f\\lambda$); (C) 400 m/s doubles $\\omega$; (D) 50 m/s quarters it (swapped $k$ and $\\omega$ roles: $k/\\omega$).`,
    ...
    diagram: {
      kind: 'wave',
      title: 'y = 0.04 sin(2π(2t − 0.01x)) at t = 0',
      waves: [
        { type: 'sine', amplitude: 40, cycles: 2, label: 'y (cm scaled: 0.04 m peak)' },
      ],
      xAxis: { label: 'x (half-wavelengths of 50 m)' },
      yAxis: { label: 'y' },
    },
  },
  {
...
**Why the others are wrong:** (A) $\\mu_0I/2\\pi R$ is the straight-wire formula misapplied at distance $R$; (C) $\\mu_0I/4R$ is the semicircle value; (D) $\\mu_0I/3R$ corresponds to $240^\\circ$ or $\\theta = 4\\pi/3$.`,
    ...
    diagram: {
      ...
      yRange: [-3, 3],
      elements: [
        { type: 'circle', cx: 0, cy: 0, r: 2, color: 'var(--chart-3)', dashed: true },
        { type: 'point', x: 0, y: 0, label: 'C (find B here)' },
        { type: 'segment', from: [0, 0], to: [1.73, 1], label: 'R', color: 'var(--gold)' },
        { type: 'segment', from: [0, 0], to: [1.73, -1], label: 'R', color: 'var(--gold)' },
        { type: 'label', x: 2.1, y: 1.2, text: 'I →', color: 'var(--chart-2)' },
        { type: 'label', x: 2.1, y: -1.4, text: 'arc 120°', color: 'var(--chart-2)' },
      ],
      square: true,
      showGrid: false,
      title: 'current arc of 120°',
    },
  },
  {
    subject: 'PHYSICS', section: 'A',
...
**Why the others are wrong:** (A) 30° is the half-angle (one side only); (B) 90° would need $\\sin\\theta = 1$, i.e. $a = \\lambda$; (C) 45° corresponds to $\\sin\\theta = 1/\\sqrt2$, i.e. $a = \\sqrt2\\lambda$.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'diffraction intensity profile (a = 2λ)',
      xAxis: { label: 'angle (°)', min: -90, max: 90, ticks: [-60, -30, 0, 30, 60] },
      yAxis: { label: 'I / I₀', min: 0, max: 1.1, ticks: [1] },
      curves: [
        { type: 'curve', color: 'var(--gold)', points: [[-90, 0.02], [-75, 0.06], [-60, 0], [-45, 0.28], [-30, 0], [-15, 0.72], [0, 1], [15, 0.72], [30, 0], [45, 0.28], [60, 0], [75, 0.06], [90, 0.02]] },
      ],
      markers: [
        { x: -30, y: 0, label: 'first minimum' },
        { x: 30, y: 0, label: 'first minimum' },
      ],
    },
  },
...
**Sanity check:** photon energy exceeds the work function by exactly 1 eV, and $1\\ \\text{eV}$ of electron energy always corresponds to a $1\\ \\text{V}$ stopping potential — the nicest conversion in physics.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'K_max vs frequency (W₀ = 2.5 eV)',
      xAxis: { label: 'ν (×10¹⁵ Hz)', min: 0, max: 1.3, ticks: [0.604, 1] },
      yAxis: { label: 'K_max (eV)', min: 0, max: 3, ticks: [1, 2, 3] },
      curves: [
        { type: 'line', color: 'var(--gold)', points: [[0.604, 0], [0.8, 0.81], [1.0, 1.64], [1.2, 2.47]] },
      ],
      markers: [
        { x: 0.604, y: 0, label: 'ν₀' },
        { x: 1.0, y: 1.64, label: 'slope = h' },
      ],
    },
  },
  {
...
**Sanity check:** the current is comfortably in the mA range expected for a small-signal diode circuit.`,
    ...
    diagram: {
      kind: 'circuit',
      components: [
        { type: 'wire', x1: 60, y1: 60, x2: 280, y2: 60 },
        { type: 'wire', x1: 60, y1: 200, x2: 280, y2: 200 },
        { type: 'wire', x1: 60, y1: 60, x2: 60, y2: 90 },
        { type: 'wire', x1: 60, y1: 130, x2: 60, y2: 200 },
        { type: 'battery', x1: 60, y1: 90, x2: 60, y2: 130, label: 'V', value: '5 V' },
        { type: 'wire', x1: 160, y1: 60, x2: 160, y2: 100 },
        { type: 'resistor', x1: 160, y1: 100, x2: 160, y2: 160, label: 'R', value: '1 kΩ' },
        { type: 'wire', x1: 160, y1: 160, x2: 160, y2: 200 },
        { type: 'wire', x1: 240, y1: 60, x2: 240, y2: 115 },
        { type: 'wire', x1: 228, y1: 115, x2: 252, y2: 115 },
        { type: 'wire', x1: 236, y1: 125, x2: 244, y2: 125 },
        { type: 'wire', x1: 240, y1: 125, x2: 240, y2: 200 },
        { type: 'junction', x: 60, y: 60 },
        { type: 'junction', x: 60, y: 200 },
      ],
      labels: ['right element = Si diode, forward drop 0.7 V'],
    },
  },
  {
    subject: 'PHYSICS', section: 'B',
