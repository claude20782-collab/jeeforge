...
**Why the others are wrong:** (B) $0.38$ comes from taking the torque arm ratio inverted ($\\tan$ instead of $\\cot$); (C) $0.50$ is the value for a $45^\\circ$ ladder; (D) $0.58 = \\cot 60^\\circ$ itself — forgetting the factor $\\tfrac12$ from the centre of mass.`,
    ...
    diagram: {
      kind: 'fbd',
      bodies: [
        { type: 'ground', x: 20, y: 238, w: 270, label: 'rough floor (μ)' },
        { type: 'wall', x: 190, y: 45, h: 195, label: 'smooth wall' },
        { type: 'string', x: 95, y: 238, w: 95, h: -164, label: 'ladder (mass m, length L)' },
      ],
      forces: [
        { from: [190, 88], to: [146, 88], label: 'N_w', color: 'var(--chart-2)' },
        { from: [142, 156], to: [142, 198], label: 'mg (at L/2)', color: 'var(--chart-3)' },
        { from: [95, 238], to: [95, 198], label: 'N_f', color: 'var(--chart-2)' },
        { from: [95, 230], to: [134, 230], label: 'f ≤ μN_f', color: 'var(--chart-3)' },
      ],
      dims: [
        { from: [95, 250], to: [190, 250], label: 'ladder at 60° to floor' },
        { from: [120, 226], to: [152, 226], label: '60°' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) zero is the net *change* of internal energy over a cycle (not the work); (C) 200 J counts only the expansion leg; (D) −100 J would be the counter-clockwise loop.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'PV cycle — clockwise rectangle',
      showGrid: true,
      xAxis: { label: 'V (L)', min: 0, max: 3, ticks: [0.5, 1, 1.5, 2, 2.5] },
      yAxis: { label: 'P (kPa)', min: 0, max: 250, ticks: [50, 100, 150, 200, 250] },
      curves: [
        { type: 'line', color: 'var(--gold)', label: 'A→B→C→D→A', points: [[1, 200], [2, 200], [2, 100], [1, 100], [1, 200]] },
      ],
      shadedRegions: [{ points: [[1, 100], [2, 100], [2, 200], [1, 200]], color: 'var(--gold)', label: 'W = 100 J' }],
      markers: [
        { x: 1, y: 200, label: 'A (1, 200)' }, { x: 2, y: 200, label: 'B (2, 200) isobaric' },
        { x: 2, y: 100, label: 'C (2, 100)' }, { x: 1, y: 100, label: 'D (1, 100) isochoric' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) $2\\pi$ uses $T = 4$ s (half period misread); (B) $4\\pi$ uses $\\omega = \\pi/2$; (D) $16\\pi$ doubles the product (uses $2\\omega v_{\\max}$).`,
    ...
    diagram: {
      ...
      curves: [
        {
          type: 'curve', color: 'var(--gold)', label: 'v = 8 sin(πt)',
          points: [[0, 8], [0.25, 5.66], [0.5, 0], [0.75, -5.66], [1, -8], [1.25, -5.66], [1.5, 0], [1.75, 5.66], [2, 8]],
        },
      ],
      markers: [
        { x: 0, y: 8, label: 'v_max = 8 cm/s' },
        { x: 1, y: -8, label: '−8' },
        { x: 0.5, y: 0, label: 'v = 0 (x = ±A)' },
        { x: 1.5, y: 0, label: 'v = 0' },
      ],
    },
  },
...
**Why the others are wrong:** (A) 20 μC is $C_3 \\times$ the series-branch fraction $10/3$ V — wrongly assuming the series branch divides voltage with $C_3$; (C) 40 μC uses $Q = C_{eq}V$ with $C_{eq} = 4$ μF; (D) 100 μC treats $C_1 \\parallel C_2$ then adds $C_3$.`,
    ...
    diagram: {
      ...
      components: [
        ...
        { type: 'junction', x: 60, y: 220 },
        { type: 'junction', x: 150, y: 60 },
        { type: 'junction', x: 150, y: 220 },
        { type: 'junction', x: 250, y: 60 },
        { type: 'junction', x: 250, y: 220 },
        { type: 'arrow', x1: 190, y1: 60, x2: 225, y2: 60, label: 'I₁ (series branch)', color: 'var(--chart-2)' },
        { type: 'arrow', x1: 250, y1: 170, x2: 250, y2: 200, label: '', color: 'var(--chart-5)' },
        { type: 'label', x: 250, y: 205, text: 'C₃ sees full 10 V → Q₃ = 60 μC' },
        { type: 'label', x: 118, y: 200, text: 'C₁,C₂ series — irrelevant to Q₃' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (B) 122 nm is the $n=2 \\to 1$ Lyman-α line ($10.2$ eV); (C) 486 nm is Balmer $4\\to2$; (D) 656 nm is Balmer $3\\to2$ (H-α, visible red) — the favourite confusion of Lyman with Balmer.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'Hydrogen energy levels (eV, to scale)',
      ...
      curves: [
        ...
        { type: 'line', color: 'var(--chart-3)', points: [[4, -13.6], [4, -1.51]], label: 'ΔE = 12.09 eV' },
      ],
      markers: [
        { x: 2, y: -0.85, label: 'n = 4' },
        { x: 2, y: -1.51, label: 'n = 3 (initial)' },
        { x: 2, y: -3.4, label: 'n = 2' },
        { x: 2, y: -13.6, label: 'n = 1 (ground)' },
        { x: 2, y: 0, label: 'n = ∞ (E = 0)' },
        { x: 4, y: -7.5, label: 'λ = 1240/12.09 ≈ 103 nm' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) 30° is the *refracted* angle $r$ inside the glass; (B) 45° has no role here; (D) 90° would give grazing incidence.`,
    ...
    diagram: {
      ...
      rays: [
        ...
        { from: [172, 150], to: [248, 162], label: 'r = 30° = A/2' },
        { from: [248, 162], to: [420, 118], label: 'e = 60° (symmetric)' },
        { from: [130, 118], to: [214, 190], dashed: true, label: 'N₁', color: 'var(--chart-3)' },
        { from: [248, 90], to: [300, 190], dashed: true, label: 'N₂', color: 'var(--chart-3)' },
        { from: [172, 150], to: [360, 229], dashed: true, label: 'undeviated path', color: 'var(--muted-foreground)' },
        { from: [330, 165], to: [370, 140], label: 'δm = 60°', color: 'var(--chart-5)' },
      ],
    },
  },
  {
...
**Sanity checks:** the shift $+37.5$ Hz is about $+6\\%$, matching $v_s/(v - v_s) = 20/320 = 6.25\\%$. If the train were receding you would get $600 \\times 340/360 = 566.7$ Hz — the symmetric drop.`,
    ...
    diagram: {
      kind: 'wave',
      title: 'Doppler: wavefronts stretched behind, compressed ahead of the moving source',
      waves: [
        { type: 'sine', amplitude: 40, cycles: 4.5, label: "behind: λ' = (v + vₛ)/f — longer", color: 'var(--chart-2)' },
        { type: 'sine', amplitude: 40, cycles: 8, phase: 1, label: "ahead: λ' = (v − vₛ)/f — shorter", color: 'var(--chart-3)' },
      ],
      xAxis: { label: 'x → (source moving right at vₛ)' },
      yAxis: { label: 'y' },
    },
  },
  {
