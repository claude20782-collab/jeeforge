...
**Why the others are wrong:** (A) 34.5% treats water as $106/286$ inverted-ish (uses the anhydrous fraction wrongly); (B) 54.9% counts only $\\tfrac{180}{328}$ — a wrong anhydrous mass (adding O instead of H₂O); (D) 72.7% = 208/286 slips an extra 28 into the water.`,
    ...
    diagram: {
      kind: 'table',
      headers: ['Species', 'Molar mass (g/mol)', 'Mass fraction'],
      rows: [
        ['Na₂CO₃ (anhydrous)', 106, '106/286 = 37.1%'],
        ['10 H₂O (water of crystallisation)', 180, '180/286 = 62.9%'],
        ['Na₂CO₃·10H₂O (washing soda)', 286, '100%'],
      ],
      caption: '% water = 180/286 × 100 ≈ 62.9% — nearly two-thirds',
      highlightCells: [[1, 2]],
    },
  },
  {
...
**Why the others are wrong:** every other option still has 1–2 unpaired $\\pi^*$ electrons — paramagnetic. Only the peroxide ion empties the unpaired spins while keeping BO = 1.`,
    ...
    diagram: {
      kind: 'table',
      headers: ['Species', 'Bond order', 'Unpaired e⁻ (in π*)', 'Magnetism'],
      rows: [
        ['O₂⁺', 2.5, 1, 'paramagnetic'],
        ['O₂', 2.0, 2, 'paramagnetic'],
        ['O₂⁻', 1.5, 1, 'paramagnetic'],
        ['O₂²⁻ (peroxide)', 1.0, 0, 'diamagnetic ✓'],
      ],
      caption: 'Dioxygen MO summary — adding e⁻ to π* lowers B.O. and pairs spins',
      highlightCells: [[3, 0], [3, 1], [3, 2], [3, 3]],
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) −0.186 °C uses molality 0.1 (forgetting the 500 g, i.e. using molarity-style dilution); (C) −1.86 °C uses 1 mol/kg; (D) −3.72 °C assumes $i = 2$ — glucose is a non-electrolyte.`,
    ...
    diagram: {
      kind: 'graph',
      title: 'Cooling curves — pure water vs glucose solution',
      showGrid: true,
      xAxis: { label: 'time →', min: 0, max: 10, ticks: [2, 5, 8] },
      yAxis: { label: 'T (°C)', min: -1, max: 3, ticks: [-0.372, 0, 1, 2] },
      curves: [
        { type: 'curve', color: 'var(--chart-2)', label: 'pure water (plateau at 0 °C)', points: [[0, 3], [1.5, 0.6], [2.5, 0.02], [5, 0], [7, -0.05], [8.5, -1.8]] },
        { type: 'curve', color: 'var(--gold)', label: 'solution (plateau at −0.372 °C)', points: [[0, 3], [1.5, -0.1], [2.5, -0.35], [5, -0.372], [7, -0.4], [8.5, -2]] },
      ],
      markers: [
      ...
      shadedRegions: [
        { points: [[3, 0], [7, 0], [7, -0.372], [3, -0.372]], color: 'var(--chart-3)', label: 'ΔTf = Kf·m' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) 1.13 V adds the log term (inverted quotient); (B) 1.10 V ignores concentrations altogether (standard conditions only); (C) 1.05 V uses n = 1.`,
    ...
    diagram: {
      kind: 'apparatus',
      parts: [
        { type: 'beaker', x: 60, y: 160, w: 95, h: 105, fill: 0.5, label: 'ZnSO₄ (0.1 M)' },
        { type: 'beaker', x: 240, y: 160, w: 95, h: 105, fill: 0.5, label: 'CuSO₄ (0.01 M)' },
        { type: 'wire', x1: 95, y1: 90, x2: 95, y2: 165, label: 'Zn (anode)' },
        { type: 'wire', x1: 300, y1: 90, x2: 300, y2: 165, label: 'Cu (cathode)' },
        { type: 'wire', x1: 95, y1: 90, x2: 300, y2: 90 },
        { type: 'tube', x: 155, y: 130, w: 90, h: 34, label: 'salt bridge (KNO₃)' },
        { type: 'arrow', x1: 130, y1: 72, x2: 265, y2: 72, label: 'e⁻ flow: Zn → Cu' },
        { type: 'arrow', x1: 165, y1: 142, x2: 195, y2: 142, label: 'K⁺ →', color: 'var(--chart-2)' },
        { type: 'arrow', x1: 235, y1: 155, x2: 205, y2: 155, label: '← NO₃⁻', color: 'var(--chart-3)' },
        { type: 'label', x: 197, y: 50, text: 'voltmeter: E = ? (n = 2)' },
        { type: 'label', x: 60, y: 280, text: 'anode: Zn → Zn²⁺ + 2e⁻ (oxidation)' },
        { type: 'label', x: 240, y: 300, text: 'cathode: Cu²⁺ + 2e⁻ → Cu (reduction)' },
      ],
      caption: 'Daniell cell Zn|Zn²⁺(0.1 M)||Cu²⁺(0.01 M)|Cu — E° = 1.10 V',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (B) 4.90 BM $= \\sqrt{24}$ ↔ 4 unpaired electrons (would be Mn³⁺/Cr²⁺ high-spin); (C) 3.87 BM $= \\sqrt{15}$ ↔ 3 unpaired (Cr³⁺); (D) 1.73 BM ↔ 1 unpaired (Ti³⁺/Fe³⁺ low-spin, e.g. in $[\\text{Fe(CN)}_6]^{3-}$).`,
    ...
    diagram: {
      kind: 'bars',
      title: 'spin-only moments μ = √n(n+2) BM of common 3d ions',
      categories: ['Ti³⁺ (d¹, n=1)', 'Cr³⁺ (d³, n=3)', 'Fe³⁺ (d⁵, n=5)', 'Mn²⁺ (d⁵, n=5)'],
      series: [
        { name: 'μ (BM) — high-spin', values: [1.73, 3.87, 5.92, 5.92], color: 'var(--gold)' },
        { name: 'n unpaired e⁻', values: [1, 3, 5, 5], color: 'var(--chart-2)' },
      ],
      yAxis: { label: 'μ (BM) / n', min: 0, max: 7 },
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) 3 misses either the branched primary or the tertiary; (C) 5 adds a phantom (2-methylbutan-... would need 5 carbons); (D) 8 counts ethers.`,
    ...
    diagram: {
      kind: 'organic',
      parts: [
        { type: 'text', x: 40, y: 30, text: 'All four C₄H₁₀O alcohol isomers:', bold: true },
        { type: 'chain', x: 40, y: 60, atoms: [{ sym: 'CH₃' }, { sym: 'CH₂' }, { sym: 'CH₂' }, { sym: 'CH₂OH' }] },
        { type: 'text', x: 40, y: 85, text: 'butan-1-ol (1°)' },
        { type: 'chain', x: 40, y: 115, atoms: [{ sym: 'CH₃' }, { sym: 'CH(OH)' }, { sym: 'CH₂' }, { sym: 'CH₃' }] },
        { type: 'text', x: 40, y: 140, text: 'butan-2-ol (2°, chiral)' },
        { type: 'chain', x: 40, y: 170, atoms: [{ sym: 'CH₃' }, { sym: 'CH(CH₃)' }, { sym: 'CH₂OH' }] },
        { type: 'text', x: 40, y: 195, text: '2-methylpropan-1-ol (1°, branched)' },
        { type: 'chain', x: 40, y: 225, atoms: [{ sym: 'CH₃' }, { sym: 'C(OH)(CH₃)' }, { sym: 'CH₃' }] },
        { type: 'text', x: 40, y: 250, text: '2-methylpropan-2-ol (3°, tert-butyl)' },
      ],
      caption: 'C₄H₁₀O monohydric alcohols — 4 structural isomers',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
...
**Why the others are wrong:** (A) acetone + formaldehyde comes from 2-methylpropene (isobutylene); (B) propanal + formaldehyde would need the CH(CH₃)CH₃ fragment on the H-side; (D) butanone would need an ethyl on the disubstituted carbon.`,
    ...
    diagram: {
      kind: 'organic',
      parts: [
        { type: 'chain', x: 40, y: 60, atoms: [{ sym: 'CH₃' }, { sym: 'C(CH₃)' }, { sym: 'CH' }, { sym: 'CH₃' }], double: [1] },
        { type: 'text', x: 40, y: 100, text: 'C₂=C₃ double bond cleaved by O₃,', bold: false },
        { type: 'text', x: 40, y: 122, text: 'each sp² carbon keeps its own substituents' },
        { type: 'arrow', x1: 210, y1: 80, x2: 290, y2: 80, label: 'O₃; Zn/H₂O (reductive)' },
        { type: 'text', x: 320, y: 60, text: '(CH₃)₂C=O', bold: true },
        { type: 'plus', x: 365, y: 85 },
        { type: 'text', x: 380, y: 60, text: 'CH₃CHO', bold: true },
        { type: 'text', x: 320, y: 100, text: 'acetone (ketone)', bold: false },
        { type: 'text', x: 380, y: 100, text: 'acetaldehyde', bold: false },
        { type: 'text', x: 40, y: 155, text: 'reductive workup ⇒ aldehyde survives (oxidative would give CH₃COOH)' },
      ],
...
**Why the others are wrong:** each candidate fails exactly one comparison above — phenol lacks the nitro boost, p-cresol has a donor, ethanol has no resonance stabilisation at all.`,
    ...
    diagram: {
      ...
      parts: [
        ...
        { type: 'text', x: 40, y: 170, text: 'the −ve charge delocalises onto NO₂ (para = 1,4-positions):', bold: false },
        { type: 'text', x: 40, y: 192, text: 'four extra resonance forms stabilise the anion → strongest acid' },
        { type: 'text', x: 40, y: 214, text: 'NO₂ is an EWG (−R resonance, −I inductive)' },
      ],
      caption: 'resonance stabilisation of p-nitrophenoxide (pKₐ ≈ 7.1 vs phenol 10.0)',
    },
  },
  {
    subject: 'CHEMISTRY', section: 'A',
