...
**Why the others are wrong:** (A) +1/6 misses the sign (sin x *lags* x near zero); (C) −1/3 doubles it; (D) 0 treats the numerator as $O(x)$-cancelled to nothing.`,
    ...
    diagram: {
      ...
      showGrid: true,
      xAxis: { label: 'x', min: -1.5, max: 1.5, ticks: [-1, -0.5, 0, 0.5, 1] },
      yAxis: { label: 'y', min: -0.2, max: 0, ticks: [-0.1667, 0] },
      curves: [
        { type: 'curve', color: 'var(--gold)', label: '(sin x − x)/x³', points: [[-1.5, -0.149], [-1, -0.1585], [-0.5, -0.166], [-0.15, -0.167], [0.15, -0.167], [0.5, -0.166], [1, -0.1585], [1.5, -0.149]] },
        { type: 'line', color: 'var(--chart-2)', dashed: true, label: 'y = −1/6 (asymptote)', points: [[-1.5, -0.1667], [1.5, -0.1667]] },
      ],
...
**Why the others are wrong:** (A) −11 is the *minimum* value (reading the wrong stationary point); (B) 11 drops a sign; (D) 27 = f(−2) with the wrong linear term.`,
    ...
    diagram: {
      ...
      yAxis: { label: 'y', min: -15, max: 25, ticks: [-11, 0, 5, 11, 21] },
      curves: [
        { type: 'curve', color: 'var(--gold)', label: 'f(x)', points: [[-4, -11], [-3, 14], [-2, 21], [-1, 16], [0, 5], [1, -6], [2, -11], [3, -4], [4, 21]] },
      ],
      markers: [
        { x: -2, y: 21, label: 'local max (−2, 21), f″ < 0' },
...
**Why the others are wrong:** (A) 4 = the $x$-offset; (B) $4\\sqrt2 = \\sqrt{32}$ (arithmetic slip in $S(P)$); (D) $2\\sqrt{13} = \\sqrt{52}$ uses $-2y$ as $+2y$.`,
    ...
    diagram: {
      ...
      showGrid: true,
      elements: [
        { type: 'circle', cx: -2, cy: 1, r: 3.16, label: 'x² + y² + 4x − 2y + 5 = 0 (r = √10)' },
        { type: 'point', x: -2, y: 1, label: 'C (−2, 1)', labelPos: 'W' },
        { type: 'point', x: 5, y: 4, label: 'P (5, 4)', labelPos: 'E' },
        { type: 'segment', from: [5, 4], to: [0.34, -1.13], label: 'L = 4√3 = √48', color: 'var(--gold)' },
        { type: 'segment', from: [5, 4], to: [-1.93, 4.16], label: 'L = 4√3', color: 'var(--gold)' },
        ...
        { type: 'segment', from: [-2, 1], to: [5, 4], label: 'PC = √58', color: 'var(--muted-foreground)', dashed: true },
        { type: 'label', x: 0.6, y: 2.4, text: 'T₁', color: 'var(--gold)' },
        { type: 'label', x: -3.6, y: 4.6, text: 'T₂', color: 'var(--gold)' },
      ],
      title: 'tangents from P: L² = |S(P)| = 48',
    },
  },
  {
    subject: 'MATHEMATICS', section: 'A',
...
**Why the others are wrong:** (B) 4/5 = $b/a$ (using the minor axis); (C) 1/2 would need $c = 2.5$; (D) 3/4 mixes $c/b$-type ratios.`,
    ...
    diagram: {
      ...
      elements: [
        ...
        { type: 'point', x: 3, y: 0, label: 'F₂ (3, 0)', labelPos: 'S' },
        { type: 'point', x: 5, y: 0, label: 'A (5, 0)', labelPos: 'SE' },
        { type: 'point', x: -5, y: 0, label: "A' (−5, 0)", labelPos: 'SW' },
        { type: 'point', x: 0, y: 4, label: 'B (0, 4)', labelPos: 'NW' },
        ...
        { type: 'segment', from: [-3, 0], to: [3, 0], label: '2c = 6', color: 'var(--chart-5)', dashed: true },
        { type: 'label', x: -5.5, y: 4.4, text: 'PF₁ + PF₂ = 10 = 2a', color: 'var(--chart-3)' },
        { type: 'label', x: 3.2, y: -4.2, text: 'e = c/a = 3/5', color: 'var(--chart-3)' },
      ],
    },
  },
  {
...
**Why the others are wrong:** (A) 0 would mean coplanar edges; (B) 1 loses a cofactor sign in the middle minor; (C) √2 mixes up with the vectors' lengths ($|\\vec a| = \\sqrt2$).`,
    ...
    diagram: {
      ...
      planes: [
        { points: [[0, 0, 0], [1, 1, 0], [1, 2, 1], [0, 1, 1]], label: 'face (a, b)', color: 'var(--gold)', opacity: 0.25 },
        { points: [[0, 0, 0], [1, 1, 0], [2, 1, 1], [1, 0, 1]], label: 'face (a, c)', color: 'var(--chart-3)', opacity: 0.25 },
        { points: [[0, 0, 0], [0, 1, 1], [1, 1, 2], [1, 0, 1]], label: 'face (b, c)', color: 'var(--chart-2)', opacity: 0.25 },
      ],
      lines: [
        { from: [1, 1, 0], to: [2, 2, 2], label: 'far edges', dashed: true, color: 'var(--muted-foreground)' },
        { from: [0, 1, 1], to: [2, 2, 2], dashed: true, color: 'var(--muted-foreground)' },
        { from: [1, 0, 1], to: [2, 2, 2], dashed: true, color: 'var(--muted-foreground)' },
      ],
      points: [
        { x: 2, y: 2, z: 2, label: 'a+b+c', color: 'var(--chart-4)' },
      ],
      showGrid: false,
    },
  },
...
**Why the others are wrong:** (A) 1 forgets the $\\sqrt9 = 3$ divisor; (C) 2/3 mis-evaluates the numerator as 2; (D) 3 is the denominator alone.`,
    ...
    diagram: {
      ...
      points: [
        ...
        { x: 0.78, y: 2.11, z: 2.78, label: 'Q (foot of ⊥)', color: 'var(--chart-2)' },
      ],
      planes: [
      ...
      vectors: [
        ...
        { from: [0.78, 2.11, 2.78], to: [1.44, 1.78, 3.44], label: 'n ∝ (2, −1, 2)', color: 'var(--chart-5)', dashed: true },
      ],
      showGrid: false,
    },
  },
...
**Why the others are wrong:** (B) 8 rounds down wrongly; (C) 9 = $\\dfrac{n^2+?}{...}$ guess; (D) 2.87 is the **standard deviation** ($\\sqrt{8.25} \\approx 2.87$), not the variance.`,
    ...
    diagram: {
      ...
      caption: 'variance = 385/10 − 5.5² = 8.25 (σ ≈ 2.87)',
      highlightCells: [[10, 0], [10, 1], [10, 2]],
    },
  },
  {
    subject: 'MATHEMATICS', section: 'A',
...
**Why the others are wrong:** (A) 1 = $(5+13-12)/2$-style mixing of sides; (C) 3 would be the exradius-scale guess; (D) 2.5 is half the smallest side.`,
    ...
    diagram: {
      ...
      showGrid: true,
      elements: [
        { type: 'polygon', points: [[0, 0], [12, 0], [0, 5]], label: '', color: 'var(--gold)', fill: 'var(--gold)' },
        ...
        { type: 'circle', cx: 2, cy: 2, r: 2, label: 'incircle', color: 'var(--chart-2)' },
        { type: 'point', x: 2, y: 2, label: 'I (incentre)', labelPos: 'NE', color: 'var(--chart-2)' },
        { type: 'segment', from: [2, 2], to: [2, 0], label: 'r = 2', color: 'var(--chart-2)', dashed: true },
        { type: 'segment', from: [0, 5], to: [12, 0], label: 'c = 13 (hypotenuse)', color: 'var(--chart-3)' },
        { type: 'angleArc', at: [0, 0], fromDeg: 0, toDeg: 90, r: 25, label: '90°' },
        { type: 'label', x: 6, y: -1.4, text: 'b = 12', color: 'var(--chart-3)' },
        { type: 'label', x: -1.5, y: 2.5, text: 'a = 5', color: 'var(--chart-3)' },
      ],
      title: '5–12–13 triangle with incircle — r = Δ/s = 30/15 = 2',
    },
  },
...
**Sanity check:** the region sits inside the 2×4 bounding box (area 8) and looks like a thin crescent — $\\approx 1.3$ is plausible.`,
    ...
    diagram: {
      ...
      showGrid: true,
      xAxis: { label: 'x', min: -0.5, max: 2.5, ticks: [0, 0.5, 1, 1.5, 2] },
      yAxis: { label: 'y', min: -0.5, max: 4.5, ticks: [1, 2, 3, 4] },
      curves: [
        { type: 'curve', color: 'var(--chart-2)', label: 'y = x²', points: [[-0.4, 0.16], [0, 0], [0.5, 0.25], [1, 1], [1.5, 2.25], [2, 4], [2.4, 5.76]] },
        { type: 'line', color: 'var(--gold)', label: 'y = 2x', points: [[-0.2, -0.4], [2.2, 4.4]] },
      ],
      shadedRegions: [{ points: [[0, 0], [2, 4], [1.5, 2.25], [1, 1], [0.5, 0.25]], color: 'var(--gold)', label: 'A = 4/3' }],
      markers: [
        { x: 0, y: 0, label: '(0, 0)' },
        { x: 2, y: 4, label: '(2, 4)' },
      ],
